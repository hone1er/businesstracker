        ...
        for idx,row in iterable:
            if idx < len(df)-1:
                ...
                if typehead == 'Service Fee' and nexttype == 'Hourly':
                    ...
                    self.db.income.insert_one({'client': {'name': client, 'job': {'name': job_description, 'date': date, 'platform': 'UpWork', 'ref_id': job_id}}, 'income': {'total': income, 'fee': {'amount': fee_amount, 'ref_id': fee_id}, 'net': net}})
